...
(define (analyze exp)
  (cond ((self-evaluating? exp) (lambda (env) exp))
        ((variable? exp) (lambda (env) (lookup-variable exp env)))
        (else (let ((analyze-proc ((eval-driver-table 'get) (car exp))))
                (cond (analyze-proc (analyze-proc exp))
                      ((application? exp)
                       (analyze-application exp))
                      (else (error "unkonwn expression! -- ANALYZE" exp)))))))


(define (analyze-application exp)
  (let ((operator-ex (analyze (operator exp)))
        (operands-ex (map analyze (operands exp))))
    (lambda (env)
      (apply-process (operator-ex env)
                     (map (lambda (ex) (ex env)) operands-ex)))))
...
(define (analyze-sequence exps)
  (define (iter cur rests)
    (if (null? rests)
        cur
        (iter (combine cur (car rests))
              (cdr rests))))
  (define (combine a b) (lambda (env) (a env) (b env)))
  (let ((pros (map analyze exps)))
    (if (null? pros)
        (error "empty sequence -- ANELYZE-SEQUENCE")
        (iter (car pros) (cdr pros)))))
...
(define (eval-and exp)
  ...
  (let ((exps (cdr exp)))
    (if (null? exps)
        true
        (lambda (env) (iter exps env)))))
(define (last-pair? exp)
  (and (pair? exp) (null? (cdr exp))))


(define (eval-quote exp)
  (lambda (env) (text-of-quote exp)))

(define (eval-lambda exp)
  (lambda (env) 
    (make-procedure (analyze-sequence (lambda-body exp))
                    (lambda-params exp)
                    env)))
...
(define (eval-define exp)
  (let ((var (definition-variable exp))
        (value (analyze (definition-value exp))))
    (lambda (env)
      (definite! var
                 (value env)
                 env))))
...
(define (eval-if exp)
  (let ((pred (analyze (if-predicate exp)))
        (conse (analyze (if-consequence exp)))
...
(define (eval-for exp)
  (analyze (for->combination exp)))

(define (eval-make-unbound! exp)
  (let ((var (make-unbound!-var exp)))
    (lambda (env)
      (unbound! env var))))

(define (eval-delay exp)
  (lambda (env)
    (make-env-delay (delay-exp exp) env)))

(define (eval-letrec exp)
  (lambda (env)
    ((analyze (letrec->let exp)) env)))
;; eval end;
