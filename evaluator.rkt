  ...
  (cond ((self-evaluating? exp) exp)
        ...
        (else (let ((eval-proc ((eval-driver-table 'get) (car exp))))
                (cond (eval-proc (eval-proc exp env))
                      ((application? exp)
                       (apply-process (actual-value (operator exp) env)
                                      (operands exp)
                                      env))
                      (else (error "unkonwn expression! -- EVAL" exp)))))))

(define (apply-process proc operands env)
  (if (primitive-procedure? proc)
      (apply-primitive-procedure proc (list-of-arg-values operands env))
      (if (compound-procedure? proc)
          (eval-sequence (procedure-body proc)
                         (extend-env (procedure-env proc)
                                     (procedure-actual-params (procedure-params proc))
                                     (list-of-delayed-args (procedure-params proc) operands env)))
          (error "unkonwn proc! -- APPLY-PROCESS" proc))))


(define (procedure-actual-params exps)
  (map (lambda (exp)
         (if (or (lazy-param? exp) (lazy-memo-param? exp))
             (lazy-param-value exp)
             exp))
       exps))

(define (lazy-param? exp)
  (and (pair? exp) (eq? (cadr exp) 'lazy)))
(define (lazy-memo-param? exp)
  (and (pair? exp) (eq? (cadr exp) 'lazy-memo)))
(define (lazy-param-value exp)
  (car exp))
...
(define (force-it obj)
  (cond ((thunk-memo? obj)
         (let ((res (actual-value (thunk-exp obj) (thunk-env obj))))
           (set-car! obj 'thunked)
           (set-car! (cdr obj) res)
           (set-cdr! (cdr obj) '())
           res))
...
(define (thunked? obj)
  (eq? 'thunked (exp-tag obj)))
(define (thunked-value obj)
  (cadr obj))

(define (thunk? obj)
  (eq? (exp-tag obj) 'thunk))
(define (thunk-memo? obj)
  (eq? (exp-tag obj) 'thunk-memo))
...
(define (list-of-delayed-args params exps env)
  (cond ((null? exps) '())
        (else (let ((param (car params))
                    (exp (car exps)))
                (cond ((lazy-param? param)
                       (cons (delay-it exp env)
                             (list-of-delayed-args (cdr params) (cdr exps) env)))
                      ((lazy-memo-param? param)
                       (cons (delay-memo-it exp env)
                             (list-of-delayed-args (cdr params) (cdr exps) env)))
                      (else
                       (cons (actual-value exp env)
                             (list-of-delayed-args (cdr params) (cdr exps) env))))))))
;; syntax
(define (exp-tag exp)
  (if (pair? exp)
