  ...
  (define (set! key new-value)
    (if (empty? table)
        ...
        (lookup key
                ...
                (lambda (pre cur)
                  (set-next! pre (cons (make-data key new-value) (next pre)))))))
  (define (dispatch m)
    (cond ((eq? m 'get) get)
          ((eq? m 'set!) set!)
          ((eq? m 'table) table)
...
(define fetch (table 'get))
(define add! (table 'set!))
(add! 'if eval-if)